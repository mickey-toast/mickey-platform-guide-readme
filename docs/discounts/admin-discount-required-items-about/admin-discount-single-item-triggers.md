...
excerpt: >-
  You can choose any individual menu item in your restaurant menu configuration
  to be a required item for a discount. The item might be one of a list of
  items, or might be part…
hidden: false
metadata:
  description: >-
    You can choose any individual menu item in your restaurant menu
    configuration to be a required item for a discount. The item might be one of
    a list of items, or might be part…
---
...
In the list of matching items, menu groups, and menus, select the item name.
...
When you select a menu item that uses size pricing, you are prompted to select the eligible sizes. On the size dialog, select the dropdown list for the sizes, then select each size that is eligible for the discount. To save the selected sizes, select **Save**.
...
Combo discounts that include modifier options apply to the least expensive modifiers for an item. For more information, see [Configuring a combo discount](https://doc.toasttab.com/doc/platformguide/adminDiscountsConfigureCombo.html).

**To select the modifiers for an item**
...
2. On the item dialog, select **Add Modifier Groups**, then select a modifier group.
...
4. Select **Select Modifiers**. In the list of modifiers, select each eligible modifier.
