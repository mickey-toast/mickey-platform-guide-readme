...
excerpt: >-
  The menu builder makes it easy to create and add modifier groups to your Toast
  menu. A modifier group contains modifiers that allow guests to customize their
  meals. For…
hidden: false
metadata:
  description: >-
    The menu builder makes it easy to create and add modifier groups to your
    Toast menu. A modifier group contains modifiers that allow guests to
    customize their meals. For…
---
...
- You can add modifier groups to menu groups, subgroups, or menu items.
   ...
   - When you add a modifier group to a menu group or subgroup, by default, it is inherited by all menu items in the group.
      ...
      You can disable inherited modifier groups for an individual item within a menu group or subgroup, by setting the item's **Inherit modifier groups?** setting to **No**. If you set a menu item's **Inherit modifier groups?** setting to **No**, it will not inherit *any* of the modifier groups assigned to *any* of its parent menu groups. For more information, see [Inheriting modifier groups from a parent menu group](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminInheritingMenuGroupsFromParentMenuGroup).

      > 📘 Note
      >
      > The remainder of this section uses the term *menu group* to refer to both menu groups and subgroups. Menu groups and subgroups have the same configuration settings and are effectively the same except for their location in the menu hierarchy. When there is a need to differentiate, the documentation does so. For more information, see [Comparing menu groups and subgroups](https://doc.toasttab.com/doc/platformguide/adminCreatingYourMenuHierarchy.html#adminComparingMenuGroupsAndSubgroups).
- To reduce the number of modifier groups you have to manage, [add existing modifier groups](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminAddingAnExistingModifierGroupToAMenuGroupOrItem) to menu groups or items whenever possible instead of creating new ones. For example, add the same Salad Dressing modifier group to both the Dinner Salad and Side Salad menu items.
- If you remove a modifier group from a menu group, it is removed for all of the menu items in that menu group. For more information, see [Removing a modifier group](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminRemovingAModifierGroup).
...
- You can add modifier groups to menu groups or menu items.
   ...
   - When you add a modifier group to a menu group, by default, it is inherited by all menu items in the menu group.
      ...
      You can disable inherited modifier groups for an individual item within a menu group or subgroup, by setting the item's **Inherit modifier groups?** setting to **No**. If you set a menu item's **Inherit modifier groups?** option to **No**, it will not inherit *any* of the modifier groups assigned to *any* of its parent menu groups. For more information, see [Inheriting modifier groups from a parent menu group](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminInheritingMenuGroupsFromParentMenuGroup).
- To reduce the number of modifier groups you have to manage, [add existing modifier groups](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminAddingAnExistingModifierGroupToAMenuGroupOrItem) to menu groups or items whenever possible instead of creating new ones. For example, add the same Salad Dressing modifier group to both the Dinner Salad and Side Salad menu items.
- You must add at least one modifier to a group before you can save the group.

**To create a new modifier group**

1. On the details page for the [menu group](https://doc.toasttab.com/doc/platformguide/adminCreatingYourMenuHierarchy.html#adminToEditAMenuGroup), [subgroup](https://doc.toasttab.com/doc/platformguide/adminCreatingYourMenuHierarchy.html#adminToEditASubgroup), or [item](https://doc.toasttab.com/doc/platformguide/adminCreatingYourMenuHierarchy.html#adminToEditAMenuItem) that you want to add a modifier group to, select **Modifier groups**.
...
4. In the **Modifier group details** section, if you use Toast POS devices, you can [customize the POS button name and color](https://doc.toasttab.com/doc/platformguide/adminAssigningPOSButtonNicknameColorinMenuBuilder.html).
5. In the **Pricing** section, [specify how the modifiers in this group are priced](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminSpecifyingModifierGroupPricing).
6. In the **Modifiers** section, [add modifiers to your modifier group](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminAddingModifierstoModifierGroup).
7. In the **Modifier group settings** section, [choose whether selecting a modifier from this group is required or optional, and, if it's optional, whether to prompt the server](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminSpecifyingModifierGroupRequiredOptional).
8. In the **Modifier group settings** section, [specify whether you want guests to be able to order more than one modifier from this group, the minimum and maximum number of modifiers to choose from, and whether a modifier can be chosen more than once](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminAllowingCustomerstoChooseOneorMoreModifiersfromModGroup).
9. In the **Channel visibility** section, [select the ordering channels where this modifier group is visible](https://doc.toasttab.com/doc/platformguide/adminSpecifyingOrderingChannelVisibility.html). A modifier group is available for ordering on the channels where it is visible.
...
1. On the details page for the [menu group](https://doc.toasttab.com/doc/platformguide/adminCreatingYourMenuHierarchy.html#adminToEditAMenuGroup), [subgroup](https://doc.toasttab.com/doc/platformguide/adminCreatingYourMenuHierarchy.html#adminToEditASubgroup), or [item](https://doc.toasttab.com/doc/platformguide/adminCreatingYourMenuHierarchy.html#adminToEditAMenuItem) that you want to add an existing modifier group to, select **Modifier groups**.
2. In the **Modifier groups** section, do one of the following:
...
The **Modifier groups** section of the [menu item details](https://doc.toasttab.com/doc/platformguide/adminCreatingYourMenuHierarchy.html#adminToEditAMenuItem) page has an **Inherit modifier groups?** setting. By default, this setting is set to **Yes**, so that the menu item inherits modifier groups from its parent menu groups.
...
For more information on inheritance, see [Understanding inheritance](https://doc.toasttab.com/doc/platformguide/platformUnderstandingInheritance.html).
...
1. On the details page for the [menu group](https://doc.toasttab.com/doc/platformguide/adminCreatingYourMenuHierarchy.html#adminToEditAMenuGroup), [subgroup](https://doc.toasttab.com/doc/platformguide/adminCreatingYourMenuHierarchy.html#adminToEditASubgroup), or [item](https://doc.toasttab.com/doc/platformguide/adminCreatingYourMenuHierarchy.html#adminToEditAMenuItem) that contains the modifier group that you want to edit, select **Modifier groups**.
2. In the **Modifier groups** section, do one of the following:
...
You use the **Pricing** section of a [modifier group's details page](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminEditingAModifierGroup) to specify how the modifiers in a group are priced:
...
- To reduce the number of modifiers you have to manage, [add existing modifiers](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#platformAddingAnExistingModifierToAModifierGroup) to modifier groups whenever possible instead of creating new ones. For example, a Breakfast Sides modifier group and a Burger Toppings modifier group could share a Bacon modifier.
...
1. In the **Modifiers** section of a [modifier group's details page](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminEditingAModifierGroup), enter the name of the first modifier in the first row.
...
3. If your modifiers are [priced individually](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminSpecifyingModifierGroupPricing), the **Modifiers** table has a **Price** column where you can enter a price for each modifier you create.
...
1. In the **Modifiers** section of a [modifier group's details page](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminEditingAModifierGroup), select the arrow next to the **+ Add** button and then choose **Add existing** from the menu.
...
7. If you are ready for guests and employees to see these menu changes, select **Publish all changes**, then select **Publish** again in the **Confirm Publish** dialog. For more information, see [Understanding when to publish your menu](https://doc.toasttab.com/doc/platformguide/adminUnderstandingWhenToPublishYourMenu.html).
...
**To change the display sequence of modifiers in a modifier group**

1. In the **Modifiers** section of a [modifier group's details page](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminEditingAModifierGroup), select a modifier's reorder icon and drag the row to its new location.
...
4. If you are ready for guests and employees to see these menu changes, select **Publish all changes**, then select **Publish** again in the **Confirm Publish** dialog. For more information, see [Understanding when to publish your menu](https://doc.toasttab.com/doc/platformguide/adminUnderstandingWhenToPublishYourMenu.html).

#### Editing a modifier's details

The **Modifiers** table displays a modifier's name and price (the **Price** column only appears if the modifiers in the group are [priced individually](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminSpecifyingModifierGroupPricing)). To set other details for a modifier, such as button color and calories, you open its **Edit modifier** dialog.

**To set other details for a modifier**

1. In the **Modifiers** section of a [modifier group's details page](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminEditingAModifierGroup), select a modifier's edit icon.
...
1. In the **Modifiers** section of a [modifier group's details page](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminEditingAModifierGroup), select a modifier's remove icon.
...
2. Select **Save**.
3. Select the **X** icon in the upper-left corner to return to the menu builder home page.
4. If you are ready for guests and employees to see these menu changes, select **Publish all changes**, then select **Publish** again in the **Confirm Publish** dialog. See [Understanding when to publish your menu](https://doc.toasttab.com/doc/platformguide/adminUnderstandingWhenToPublishYourMenu.html) for more information.

### Specifying if a modifier group is required or optional

In the **Modifier group settings** section of a [modifier group's details page](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminEditingAModifierGroup), you can choose whether selecting a modifier from a group is required or optional:
...
In the **Modifier group settings** section of a [modifier group's details page](https://doc.toasttab.com/doc/platformguide/adminAddingModifierGroupsAndModifiers.html#adminEditingAModifierGroup), you can specify whether guests can order more than one modifier from a modifier group.
...
When you configure a modifier group to use the **What is the minimum number a guest must select** setting, you must also pay careful attention to the [ordering channel visibility](https://doc.toasttab.com/doc/platformguide/adminSpecifyingOrderingChannelVisibility.html) settings for the modifiers in that group. Specifically, you must ensure that the number of modifiers that are visible in any given ordering channel meet or exceed the minimum number of modifiers a guest or server must select from the modifier group.
...
1. On the details page for the [menu group](https://doc.toasttab.com/doc/platformguide/adminCreatingYourMenuHierarchy.html#adminToEditAMenuGroup), [subgroup](https://doc.toasttab.com/doc/platformguide/adminCreatingYourMenuHierarchy.html#adminToEditASubgroup), or [item](https://doc.toasttab.com/doc/platformguide/adminCreatingYourMenuHierarchy.html#adminToEditAMenuItem) that contains the modifier group that you want to remove, select **Modifier groups**.
