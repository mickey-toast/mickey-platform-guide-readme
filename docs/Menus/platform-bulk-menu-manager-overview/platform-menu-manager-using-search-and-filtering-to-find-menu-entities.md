...
excerpt: >-
  The Menu manager page includes a set of searching and filtering controls that
  you can use to refine the list of menu entities that are displayed, making
  individual entities…
hidden: false
metadata:
  description: >-
    The Menu manager page includes a set of searching and filtering controls
    that you can use to refine the list of menu entities that are displayed,
    making individual entities…
---
...
- [Filtering by location (multi-location only)](https://doc.toasttab.com/doc/platformguide/platformMenuManagerUsingSearchAndFilteringToFindMenuEntities.html#platformMenuManagerFilteringByLocation)
- [Filtering menu items and modifiers](https://doc.toasttab.com/doc/platformguide/platformMenuManagerUsingSearchAndFilteringToFindMenuEntities.html#platformMenuManagerFiteringMenuItemsAndModifiers)
- [Filtering price levels](https://doc.toasttab.com/doc/platformguide/platformMenuManagerUsingSearchAndFilteringToFindMenuEntities.html#platformMenuManagerFilteringPriceLevels)
- [Resetting the filters](https://doc.toasttab.com/doc/platformguide/platformMenuManagerUsingSearchAndFilteringToFindMenuEntities.html#platformMenuManagerResettingTheFilters)
...
By default, the locations control is set to the location you are currently logged into (also known as the [session restaurant](https://doc.toasttab.com/doc/platformguide/sessionRestaurant.html)). The locations you have selected remain in place as you navigate between the **Items**, **Modifiers**, and **Price levels** views. Currently, when you go to **Full menu** view, the locations control is reset to the session restaurant.

The locations control is filtered so that you can only select to view menu data for branches of the location group hierarchy where you have the **4. Restaurant Admin > 4.5 Edit Full Menu** or the **4. Restaurant Admin > 4.14 Local Menu Edit** permission. For example, consider the following location group hierarchy:
...
If you have the **Edit Full Menu** or **Local Menu Edit** permission to the Northeast Franchises location group, then you see the Corporate group, the Northeast Franchises group, the Boston location, and the New York City location in the locations control. You cannot see the Southeast Franchises group, the Atlanta location, and the Charlotte location in the locations control, because you do not have the **Edit Full Menu** or **Local Menu Edit** permission to any of the location groups or locations in that branch.
...
- For multi-location restaurants, the location control is reset so that the location you are logged into (also called the [session restaurant](https://doc.toasttab.com/doc/platformguide/sessionRestaurant.html)) is selected.
