...
excerpt: >-
  The menu manager's bulk editing feature lets you quickly make the same edit to
  multiple menu entities.
hidden: false
metadata:
  description: >-
    The menu manager's bulk editing feature lets you quickly make the same edit
    to multiple menu entities.
---

The menu manager's bulk editing feature lets you quickly make the same edit to multiple menu entities.

Currently, the bulk edit feature supports setting the same [visibility settings](https://doc.toasttab.com/doc/platformguide/platformColumnsInTheMenuManagerViews.html#platformMenuManagerItemAndModifierColumns_Visibility) for multiple menu items at once. Visibility settings control where a menu item is visible and available for ordering.

**To set the same visibility settings for multiple menu items at once**
...
4. Optionally, use the [search](https://doc.toasttab.com/doc/platformguide/platformMenuManagerUsingSearchAndFilteringToFindMenuEntities.html#platformMenuManagerSearchingForMenuEntities) and [filter controls](https://doc.toasttab.com/doc/platformguide/platformMenuManagerUsingSearchAndFilteringToFindMenuEntities.html#platformMenuManagerFilteringMenuEntities) to find the menu items you want to edit.
...
7. Toggle the visibility settings so the menu items are visible on the channels you want:
   ...
   - **Customer invoices**: The menu items are visible when creating customer invoices for orders using the **Finance > Payments > Customer invoices** page. Note that this setting is always enabled for all menu entities and cannot be changed. (For information on creating customer invoices, see this [Toast Support article](https://support.toasttab.com/article/How-to-Send-an-Invoice).)
...
9. After making your visibility changes, you must either schedule them to be published in the future, or save and publish them immediately. For more information, see [Publishing menu manager changes](https://doc.toasttab.com/doc/platformguide/platformPublishingMenuManagerChanges.html).
