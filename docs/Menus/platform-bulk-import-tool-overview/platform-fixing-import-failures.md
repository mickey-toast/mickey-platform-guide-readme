...
excerpt: >-
  There are two types of failures that can occur when importing data with the
  bulk import tool: data validation failures and import operations failures.
hidden: false
metadata:
  description: >-
    There are two types of failures that can occur when importing data with the
...
**To fix and re-import a bulk import .csv file**
...
10. If the re-import was to fix a partially complete import, you may also need to to use Toast Web to re-order the imported menu entities. For more information, see [Fixing the order of re-imported menu entities](https://doc.toasttab.com/doc/platformguide/platformFixingImportFailures.html#platformFixingTheOrderOfReImportedMenuEntities) below.
...
You can quickly restart the re-import process by selecting the **View import history** link in the notification or by going to the **Import history** tab. Any failed imports are listed in the **Incomplete imports** table. Select the **Fix and retry** link for a failed import to re-open the **Incomplete import** page for the import and continue with the [procedure](https://doc.toasttab.com/doc/platformguide/platformFixingImportFailures.html#platformToFixAndReImportABulkImportCsvFile) provided above.
