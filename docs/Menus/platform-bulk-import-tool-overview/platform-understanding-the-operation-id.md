...
excerpt: >-
  You must provide a unique operation ID for every row in a bulk import
  spreadsheet. A row's operation ID allows other rows in the spreadsheet to
  attach to it. For example,…
hidden: false
metadata:
  description: >-
    You must provide a unique operation ID for every row in a bulk import
