...
excerpt: >-
  The Price editor page displays prices for all of your menu groups and menu
  items in one location, where you can quickly view and edit them. It also
  provides features for bulk…
hidden: false
metadata:
  description: >-
    The Price editor page displays prices for all of your menu groups and menu
...
To view the **Price editor** page, [access Toast Web](https://doc.toasttab.com/doc/platformguide/adminAccessToastAdminBackend.html) and then choose **Menus > Bulk management > Price editor**. Select the arrow next to a menu or menu group to expand it and see its children. Select a menu entity's name to view its details page.
...
Icons in the area to the left of the **Price** column indicate the pricing strategy used for a menu group or item when a strategy other than [Base Price](https://doc.toasttab.com/doc/platformguide/adminBasePrice.html) is used. In the example above, the Grilled Cheese Sandwich uses the **Base Price** strategy so it has no pricing strategy icon. The examples below show the icons used for the other pricing strategies.

For menu groups and items that use the:

- [Size Price](https://doc.toasttab.com/doc/platformguide/adminSizePrice.html) strategy, each size appears on a separate row. In the example below, the small (SM) size of the Fries menu item is $2 and the large (LG) size is $4.
...
- [Time Specific Price](https://doc.toasttab.com/doc/platformguide/adminTimeSpecificPrice.html) strategy, the base price appears in the first row and the prices for other time periods appear in rows beneath the base price. In the example below, the Pasta Special is $9 between 12p.m. and 2p.m. and $10 during the rest of the day.
...
- [Menu Specific Price](https://doc.toasttab.com/doc/platformguide/adminMenuSpecificPrice.html) strategy, a menu icon appears next to the price and the price reflects the menu where that price is appearing. For example, in the illustration below, the Steak menu item costs $9 when it is ordered from the Breakfast menu.
...
- [Open Price](https://doc.toasttab.com/doc/platformguide/adminOpenPrice.html) strategy, the word "open" appears in the **Price** column. For example, the Breakfast Specials menu group below is configured to use open pricing.
...
- [Price Levels](https://doc.toasttab.com/doc/platformguide/adminUsingPriceLevels.html), the **Price editor** page shows a price levels icon and the name of the price level but it does not display the price. To see the price, you must select the price level name to view the price level's details page.
...
**To make bulk edits using the Price Editor**
...
4. Select the menu groups and items you want to edit by selecting the box on the left side of the menu group or item's row. If you select a menu group, all of its menu items and sub-groups are automatically selected.
5. Select the **Update** menu and choose the type of update you want to make:
...
**To convert a menu item to use a menu-specific price**

1. [Access Toast Web](https://doc.toasttab.com/doc/platformguide/adminAccessToastAdminBackend.html).
2. Choose **Menus > Bulk management > Price editor** to open the **Price editor** page.
3. Expand the menus and menu groups to view the menu item you want to convert.
4. Select the menu item by selecting the box on the left side of the menu item's row.
5. Select the **Update** menu and choose **Convert to Menu-Specific Price**.

   "Menu-specific price" appears to the left of the **Price** column to indicate that the pricing strategy of the menu item has changed. The **Base Price** that was defined for the menu item becomes the [default base price](https://doc.toasttab.com/doc/platformguide/adminMenuSpecificPrice.html#adminDefaultBasePriceForMenuSpecificPrice) of the **Menu Specific Price** pricing strategy.
6. Select **Save** to save the pricing strategy change, after which you can set the individual menu-specific prices.
7. Locate the menu item in a menu. If the default base price is acceptable, no changes are necessary. If the base price is not acceptable, edit the menu-specific price and select **Save**. Repeat this step for each instance of the menu item (for example, if a menu item is included in both the Lunch and Dinner menus, you perform this step twice).

   > 📘 Note
   >
   > An issue in the Toast platform prevents you from editing all of the menu-specific prices for a converted menu item in one action. You must edit each instance of the menu item individually and select **Save** after each edit.
8. [If you are done editing prices for your menus, publish your changes.](https://doc.toasttab.com/doc/platformguide/adminNoteAboutPublishingPrices.html)
...
**To convert a menu item to use a location-specific price**

1. [Access Toast Web](https://doc.toasttab.com/doc/platformguide/adminAccessToastAdminBackend.html).
2. Choose **Menus > Bulk management > Price editor** to open the **Price editor** page.
3. Expand the menus and menu groups to view the menu item you want to convert.
4. Select the menu item by selecting the box on the left side of the menu item's row.
5. Select the **Update** menu and choose **Convert to Location-Specific Price**.
6. Select **Save** and *refresh the page*. This enables the location-specific price settings for the menu item, where you can add more location-specific prices. For more information about using these settings, see [To set location-specific prices for a menu item using the Price Editor](https://doc.toasttab.com/doc/platformguide/adminEnablingAndSettingLocationSpecificPrices.html#adminToSetLocationSpecificPricesOnAMenuItem).
