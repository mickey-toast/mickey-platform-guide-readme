...
excerpt: >-
  This section provides procedures and information about configuring kitchen
  tickets.
hidden: false
metadata:
  description: >-
...
In addition, this [Toast Support article](https://support.toasttab.com/article/Customize-the-Appearance-of-KDS-Tickets) provides an overview of customizing colors for KDS devices.
...
2. Select **Full menu** view, then go to the menu item you want to edit. Select the item to see its side pane.
...
2. [Access Toast Web](https://doc.toasttab.com/doc/platformguide/adminAccessToastAdminBackend.html) and then choose **Menus > Bulk management > Advanced properties**.
3. (Multi-location restaurants only) Choose the locations you want to view from the **You are viewing** menu.
4. Expand the advanced properties table to find the modifier you want to edit, select its name, then select **Edit Item** to see the classic details page for the modifier's [item reference](https://doc.toasttab.com/doc/platformguide/adminPricingModifierOptions.html#adminUnderstandingAModifierOptionsItemReference).
5. Next to the **KDS Color**, select the square tile. The [color palette](https://doc.toasttab.com/doc/platformguide/platformKitchenConfiguringTickets.html#adminKdsColorPalette) opens.
...
You can choose to sort items by assigned seat numbers, assigned sort order, or by how the items are entered on the order screen. You can use the [**Item Sorting Priority**](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configItemSortingPriority) setting to sort items on kitchen tickets.

- To sort by assigned seat numbers, you must enable the seat numbers modifier group with the **Additional Modifier Groups** setting, and assign the **Seat Number** modifier group to the menu item. If two items have the same seat number, they are sorted by **Sort Order**, if available. If no **Sort Order** is available, the items are sorted by how they were added to the order. For more information, see **[Additional Modifier Groups](https://doc.toasttab.com/doc/platformguide/adminUiOptionsReference.html#configAdditionalModifierGroups)**.
...
- **Sort Order**
   ...
   > 📘 Note
   >
   > Using the **Sort Order** setting to sort modifiers on kitchen tickets is in limited release. To access this feature, send an email to `<kitchenfeedback@toasttab.com>`.
...
1. If [**Modifier display mode**](https://doc.toasttab.com/doc/platformguide/adminUiOptionsReference.html#configModifierDisplayMode) is set to:
...
3. If no **Sort Order** is configured for modifiers and **[Modifier ordering priority](https://doc.toasttab.com/doc/platformguide/adminUiOptionsReference.html#configModifierOrderingPriority)** is set to **On**, the modifiers are sorted by modifier group.
...
4. The modifiers are then ordered based on the [**Modifier group sorting**](https://doc.toasttab.com/doc/platformguide/adminUiOptionsReference.html#configModifierGroupSorting) setting option:
...
The Toast platform allows you to have separate tickets for each item in an order, or to have an order ticket with all items in an order. For example, you have an order with two items, Panini and Burger that both route to the Hot prep station. If using individual item tickets, this results in two tickets, one with Panini and another with Burger. If not using individual item tickets, this results in a single ticket with two items, Panini and Burger. The **Individual Item Tickets** setting has separate configurations for both [printed](https://doc.toasttab.com/doc/platformguide/platformKitchenConfiguringTickets.html#platformKitchenIndividualItemTicketsPrinted) and [KDS tickets](https://doc.toasttab.com/doc/platformguide/platformKitchenConfiguringTickets.html#platformKitchenIndividualItemTicketsKDS). Individual item tickets can apply to printed prep station tickets, and to KDS prep station and expediter tickets, depending on your setup.

> 📘 Note
>
> Individual tickets apply differently for items with multiple quantities. Depending on your **Individual Item Tickets** setting options, you can choose to specify whether items with multiple quantities are consolidated into a single item with a multiple quantity, or as separate items each with a quantity of one. For more information, see [Consolidate identical items](https://doc.toasttab.com/doc/platformguide/platformKitchenConfiguringTickets.html#platformKitchenConsolidateIdenticalItems).

The **Individual Item Tickets** setting has two parts for [Print Settings](https://doc.toasttab.com/doc/platformguide/platformKitchenConfiguringTickets.html#platformKitchenIndividualItemTicketsPrinted) and [KDS Settings](https://doc.toasttab.com/doc/platformguide/platformKitchenConfiguringTickets.html#platformKitchenIndividualItemTicketsKDS).
...
> 📘 Note
>
> Individual item tickets and overview tickets only apply to printed prep station tickets. Printed expediter tickets always include all items in a course for an order when configured to [send to the expediter](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configSendTo).
...
> 📘 Note
>
> Items only appear on the expediter KDS device if the prep station they are assigned to is configured to [send to the expediter](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configSendTo).
...
Choosing how to configure individual items for printed and KDS tickets can affect how identical items are consolidated. For more information, see [Consolidate identical items](https://doc.toasttab.com/doc/platformguide/platformKitchenConfiguringTickets.html#platformKitchenConsolidateIdenticalItems)Consolidating identical items.

**To configure individual item tickets for printed and KDS tickets**

1. [Access Toast Web](https://doc.toasttab.com/doc/platformguide/adminAccessToastAdminBackend.html).
2. Go to the **Kitchen > Printers, tickets, & KDS devices > Kitchen and ticket setup** page.
...
4. Select an option for **Print Settings**.
   ...
   For more information about these options, see [Print settings](https://doc.toasttab.com/doc/platformguide/platformKitchenConfiguringTickets.html#platformKitchenIndividualItemTicketsPrinted).
5. Select an option for **KDS Settings**.
   ...
   For more information about these options, see [KDS settings](https://doc.toasttab.com/doc/platformguide/platformKitchenConfiguringTickets.html#platformKitchenIndividualItemTicketsKDS).
...
If you choose to use [individual item kitchen tickets](https://doc.toasttab.com/doc/platformguide/platformKitchenConfiguringTickets.html#platformKitchenIndividualItemKitchenTickets), each item has a ticket. For this example, consolidated items appear as one ticket for Panini with a quantity of two, while unconsolidated items appear as two tickets for Panini with a quantity of one.
...
**To configure consolidated items with multiple quantities**
