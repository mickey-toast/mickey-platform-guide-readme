...
excerpt: >-
  Before a service charge can be automatically applied to a check, the Toast
  platform has to make a variety of calculations, including the following:
hidden: false
metadata:
  description: >-
    Before a service charge can be automatically applied to a check, the Toast
...
- Calculate whether the service charge is eligible to be automatically applied to a check. See [Service charge eligibility](https://doc.toasttab.com/doc/platformguide/adminServiceChargeEligibility.html). Keep in mind that you can manually apply a service charge to a check.
- Calculate how the application of discounts affects the service charge fee. See [Post-discount and pre-discount calculations](https://doc.toasttab.com/doc/platformguide/adminServiceChargeCalculation.html#adminPostDiscount).
...
- Calculate the tax on the service charge (if it is configured for taxation). See [Service charge taxes](https://doc.toasttab.com/doc/platformguide/adminServiceChargeCalculation.html#adminServiceChargeTaxes).
...
You can optionally configure a service charge to be taxed, using one or more of your existing tax rates. The following example of a service charge configuration page shows that the **Taxed** option is set to **Yes** and the **Applicable Taxes** option (in the **Taxes** section) is using the `State
...
**Tax calculation example for one tax rate**
...
**Tax calculation example for two tax rates**
...
For ****fixed percent**** service charges, the **Tip / Tax** option (in the [Tipping](https://doc.toasttab.com/doc/platformguide/adminUiOptionsReference.html#adminTippingRef) section of the **Front of house > Order screen setup > UI options** page) affects how fixed percent service charges are taxed. The option lets you specify whether a fixed percent service charge is calculated on a pre-tax basis (that is, first calculate the service charge on the net amount and then add the tax afterwards) or on a post-tax basis (that is, first add the tax to the net amount and then calculate the service charge on that amount). The calculated tax is then included in the **Tax** field on the payment screen.
