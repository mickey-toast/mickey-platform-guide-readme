...
excerpt: >-
  Cash management is an umbrella term that describes the many workflows and
  features that you can use to manage your cash transactions, report cash tips,
  and close out your day.
hidden: false
metadata:
  description: >-
    Cash management is an umbrella term that describes the many workflows and
    features that you can use to manage your cash transactions, report cash
    tips, and close out your day.
---

> ❗️ Important
>
> This content will be deprecated. For the latest information about cash drawers, see [Cash drawers](https://doc.toasttab.com/doc/platformguide/adminCashDrawers.html).
...
You can assign a cash drawer name when you [create or update a cash drawer](https://doc.toasttab.com/doc/platformguide/platformDeprecatedCashManagement.html#platformCashDrawerCreate) with the **Finance > Related > Settings > Printers & cash drawers** page in Toast Web.
...
An employee handling cash payments (either cash in drawer or cash in hand) must have the [1.6 Apply Cash Payments](https://doc.toasttab.com/doc/platformguide/adminPermissions.html#permissionApplyCashPayments) permission. In addition, the employee's Toast POS device must have the **Allow Cash Payments** device setting enabled, as described in the following procedure.
...
**To re-assign a cash drawer to a Toast POS device**
...
At the bottom of the screen are buttons for cash drawer operations, such as making a Cash Drop entry and closing the cash drawer. These operations are described in [Cash drawer operations](https://doc.toasttab.com/doc/platformguide/platformDeprecatedCashManagement.html#platformCashDrawerOperations).
...
The top pane shows three balance amounts for the closed drawer:
...
As the example shows, the **During Shift** buttons are disabled. The **End of Shift** buttons are described in [End of Shift operations](https://doc.toasttab.com/doc/platformguide/platformDeprecatedCashManagement.html#platformEndOfShiftOperations) and the **Adjust Start** button is described in [Adjust Starting Balance operation](https://doc.toasttab.com/doc/platformguide/platformDeprecatedCashManagement.html#platformAdjustStartOperation).
...
**To make a Pay Out entry from a specific cash drawer**
...
**To make a Pay Out entry from the Cash Management section**

1. On the device, tap the Toast logo in the top corner until the Toast POS home screen appears.
2. Go to the **Cash Management** section and select **Pay Out**.
...
1. [Access Toast Web](https://doc.toasttab.com/doc/platformguide/adminAccessToastAdminBackend.html).
2. Choose **Finance > Related > Settings > No sale reasons** to open the **No sale reasons** page.
...
After you close the drawer, it appears on the **Closed** tab of the **Cash Drawers** screen. For more information on closed drawers, see [Closed drawers](https://doc.toasttab.com/doc/platformguide/platformDeprecatedCashManagement.html#platformClosedCashDrawers).

**To configure cash drawer closeout thresholds**

1. [Access Toast Web](https://doc.toasttab.com/doc/platformguide/adminAccessToastAdminBackend.html).
2. Choose **Finance > Related > Settings > Cash overview > Cash drawers > Cash drawer variance** to open the **Cash drawer variance** page.
...
For more information of these configuration settings, see [Creating cash drawers](https://doc.toasttab.com/doc/platformguide/platformDeprecatedCashManagement.html#platformCashDrawerCreate).
...
**To access the cash drawer operations in Toast Web**

1. [Access Toast Web](https://doc.toasttab.com/doc/platformguide/adminAccessToastAdminBackend.html).
2. Choose **Finance > Related > Settings > Cash overview > View related reports > Cash drawer history report**to open the **Cash Drawer History** report. You can also choose **Reports > Cash and loss management > Drawer history** to open the **Cash Drawer History** report.
...
These operations are functionally similar to those on the Toast POS device, which are described in [During Shift operations](https://doc.toasttab.com/doc/platformguide/platformDeprecatedCashManagement.html#platformDuringShiftOperations). Note that the user name displayed in the cash drawer for the action is the name of the employee who is logged into Toast Web.

**To make a cash entry from Toast Web**
...
You can close an active cash drawer from Toast Web. While similar to the [close drawer operation on the Toast POS device](https://doc.toasttab.com/doc/platformguide/platformDeprecatedCashManagement.html#platformCloseDrawerOperation), the Toast Web version has the following differences:
...
**To close an active drawer from Toast Web**

1. If you need to count the money in the cash drawer, issue a [No Sale](https://doc.toasttab.com/doc/platformguide/platformDeprecatedCashManagement.html#platformNoSaleOperation) command from the Toast POS device.
...
After you close the drawer, it appears on the **Closed** tab of the **Cash Drawers** screen (on the Toast POS device) and its status is changed to **Closed** on the **Cash Drawer History** report. For more information on closed drawers, see [Closed drawers](https://doc.toasttab.com/doc/platformguide/platformDeprecatedCashManagement.html#platformClosedCashDrawers).

##### Adjusting starting balance from Toast Web

You can adjust the starting balance for a cash drawer from the Toast Web. This operation is functionally the same as the version on the Toast POS device, described in [Adjust Starting Balance operation](https://doc.toasttab.com/doc/platformguide/platformDeprecatedCashManagement.html#platformAdjustStartOperation).

**To adjust a cash drawer's starting balance from Toast Web**
...
**To undo a cash entry on the Toast POS device**
...
When you undo cash entries from Toast Web, the operations do not open the physical cash drawer. Therefore, if you want to manually add or remove cash, you must issue a [No Sale](https://doc.toasttab.com/doc/platformguide/platformDeprecatedCashManagement.html#platformNoSaleOperation) command from the Toast POS device.

**To undo a cash entry from Toast Web**
...
3. Select the **Customize shift review** button. Continue to the **Require shift review** screen, go to the **Require shift review**setting and select the **Shift review is required** option.
4. Continue to the **Close cash drawer** screen, go to the **Lock employee to cash drawer** setting and select the **Required - Employees assigned a drawer-lock job are required to lock themselves to a cash drawer** option.
5. Save and publish your changes.
