...
excerpt: >-
  Some states have sales taxes that are determined using tax tables made up of
  tax brackets. Each tax bracket consists of a dollar amount range and an
  associated tax amount for…
hidden: false
metadata:
  description: >-
    Some states have sales taxes that are determined using tax tables made up of
...
5. Use the **Tax Table** to build your Toast tax table:
   ...
   6. Clean up your tax table by removing the tax brackets that occur after the first pattern set. Start by removing the last tax bracket in the Toast tax table. To remove a tax bracket, select the trash can icon in the **Remove** column of the tax bracket and then select **Done**.
   ...
   7. Test your tax table with the **Calculate Your Tax** calculator. Enter any dollar amount in the **Menu Item Price** field and select **Calculate**. The system uses your tax table to calculate the sales tax that is shown in the **Tax Applied** field, as shown in the following example. Check that the applied tax matches the state tax table.
