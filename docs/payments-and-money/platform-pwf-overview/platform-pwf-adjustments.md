...
title: Adjustments
excerpt: >-
  Voiding a check, item or payment is the process of removing an item or payment
  from a check or removing the check altogether. Check items can only be voided
  if that have been…
hidden: false
metadata:
  description: >-
    Voiding a check, item or payment is the process of removing an item or
    payment from a check or removing the check altogether. Check items can only
    be voided if that have been…
---
...
- If a check has been sent to the kitchen, and then voided, the voided check is still shown on the **Payments** report in Toast Web. You can find this report from **Reports** > **Reports** > **Payments** > **Payments**.
...
Refunding a guest is one of the most common actions taken on a point of sale system. Most commonly, a refund is issued if an incorrect payment type is used, or to resolve guest issues. For more information about refunds, see [Refunds](https://doc.toasttab.com/doc/platformguide/adminRefundPermissionsLimitations.html).
