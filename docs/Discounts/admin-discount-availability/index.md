...
excerpt: >-
  For any discount, you can configure time-based availability criteria that
  controls when that discount is available for use:
hidden: false
metadata:
  description: >-
    For any discount, you can configure time-based availability criteria that
...
**To add availability times to a discount**

1. [Add a discount or edit an existing one](https://doc.toasttab.com/doc/platformguide/adminDiscountsDesigningDiscounts.html).
...
3. To configure a **Dates Available** date range:
   ...
   1. Select the calendar icon for the start date. Use the date picker to select the date when this discount starts to be available.
   2. Select the calendar icon for the ending date. Use the date picker to select the last date when the discount is available.
...
4. Under **Days of the Week Available**, to indicate when the discount is available, select a day to change its availability.
