...
excerpt: >-
  You can optionally add one or more promo codes to any type of discount that is
  not auto-applied. A promo code is a text string or a scannable barcode that is
  assigned to a…
hidden: false
metadata:
  description: >-
    You can optionally add one or more promo codes to any type of discount that
    is not auto-applied. A promo code is a text string or a scannable barcode
...
2. To add a new promo code, select **Add Promo Code**. An empty promo code row is added to the list.

   ![Promo Codes list with a new promo code added.](https://doc.toasttab.com/doc/media/discount-promo-code.png)

   To edit an existing promo code, select the field that you want to edit.
3. In the **Code** field, enter the text string for the promo code. You can also scan a barcode into this field.

   For more information about code values, see [Code value](https://doc.toasttab.com/doc/platformguide/adminDiscountPromoCodes.html#adminDiscountPromoCodeCode).
...
5. To make a promo code single use, check the **Single Use** check box.
   ...
   For more information about single-use promo codes, see [Single use promo codes](https://doc.toasttab.com/doc/platformguide/adminDiscountPromoCodes.html#adminDiscountPromoCodeSingleUse).
6. Optionally, set the date range during which the promo code is valid.

   To set the **Start Date** or **End Date**, select the calendar icon, then use the date picker to select the date.

   To clear the configured **Start Date** and **End Date**, select **Clear Date**.

   For more information about the effective dates for promo codes, see [Promo code effective dates](https://doc.toasttab.com/doc/platformguide/adminDiscountPromoCodes.html#adminPromoCodeDates).
7. Save and publish your changes.

#### Removing a promo code

To remove a promo code from the list, select the delete icon for the promo code. The delete icon is in the **Delete** column at the right of the promo code list.
...
**To enable promo code search for a restaurant**
...
**To use promo code search on the Toast POS app**

1. On the order screen, to display the overflow menu, select the ⋮ icon). From the overflow menu, select the **Promo code** option.
