...
excerpt: >-
  You can create discounts that automatically apply to a check when a restaurant
  employee saves items (with the Send, Stay, Hold, or Pay buttons) or prints a
  receipt.
hidden: false
metadata:
  description: >-
    You can create discounts that automatically apply to a check when a
    restaurant employee saves items (with the Send, Stay, Hold, or Pay buttons)
    or prints a receipt.
---
...
- BOGO discounts
- Combo discounts
- Bulk discounts. These are fixed currency check-level discounts and fixed percent check-level discounts that are configured with at least one required item, and are configured as [bulk discounts](https://doc.toasttab.com/doc/platformguide/adminBulkDiscounts.html).
...
3. When you are finished, to hide the **Order** column, select the **Order** button.
