...
excerpt: >-
  Configuring menu-specific prices for a menu that has multiple versions follows
  the same basic procedure as configuring menu-specific prices for a menu that
  has only one…
hidden: false
metadata:
  description: >-
    Configuring menu-specific prices for a menu that has multiple versions
    follows the same basic procedure as configuring menu-specific prices for a
    menu that has only one…
---
...
**To create menu-specific prices for a menu item on a versioned menu**
...
5. Because these changes affect multiple locations, you publish them using the **Publish Config** page, where you can publish changes to multiple locations at once.
   ...
   - In the **Locations** table, select the locations that are affected by the pricing changes you have made.
      ...
      > 📘 Note
      >
      > If you are unsure about which locations are affected by your changes and require publishing, see [About the Number column](https://doc.toasttab.com/doc/platformguide/platformAboutTheNumberColumn.html) for information on using the **Number** column to determine which locations require publishing.
   - Select **Publish Select Restaurants**.

   > 📘 Note
   >
   > For more info on the **Publish Config** page, see [Publishing changes for multiple locations](https://doc.toasttab.com/doc/platformguide/publishingChangesForMultipleLocations.html).
