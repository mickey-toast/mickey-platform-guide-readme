...
excerpt: >-
  If you use KDS devices to display tickets, you must have the 6. Web Setup >
  6.2 Kitchen / Dining Room Setup access permission to configure each device to
  display the tickets…
hidden: false
metadata:
  description: >-
    If you use KDS devices to display tickets, you must have the 6. Web Setup >
    6.2 Kitchen / Dining Room Setup access permission to configure each device
    to display the tickets…
---
...
Prep stations with the **Send to** setting set to **Expediter only** do not print or show items on prep station KDS devices. Prep stations with the **Send to** setting set to **Prep station and expediter** or **Prep station only** display on prep station KDS devices. If you use the **Sequenced Prep to Expo Fulfillment** setting, which requires fulfillment at prep stations before expediter devices, items must be fulfilled at a prep station first. If you set the **Send to** setting to **Prep station only**, however, items do not require fulfillment at the expediter.
...
For more information about the **Send to** setting, see [Sequencing fulfillment by station type and level](https://doc.toasttab.com/doc/platformguide/platformKitchenUnderstandingItemFulfillmentKDS.html#platformKDSSequencingFulfillmentByStationTypeAndLevel). For more information about the **Sequenced Prep to Expo Fulfillment** setting, see [Sending items to prep stations, expediters, or both](https://doc.toasttab.com/doc/platformguide/adminRoutingToPrepStations.html#platformKitchenSendingToPrepStationExpediterBoth).

**To assign a prep station to a KDS device**
...
When you [configure production items](https://doc.toasttab.com/doc/platformguide/adminAssignPrepStationKDS.html#adminConfigureProductionItem) for your restaurant, you create a customized system for tracking your most critical ingredients. The KDS reports a continuously updated total across fired orders for each production item below the individual tickets, including when the count is zero (0). You specify which production items you want to track on each of your KDS devices.
...
1. [Create a production item](https://doc.toasttab.com/doc/platformguide/adminAssignPrepStationKDS.html#adminCreateProdItem) for each ingredient that you want to track on the KDS.
2. [Assign the production item](https://doc.toasttab.com/doc/platformguide/adminAssignPrepStationKDS.html#adminAssignProdItemQuantity) and its quantity to a menu item or modifier.
3. [Specify the production items](https://doc.toasttab.com/doc/platformguide/adminAssignPrepStationKDS.html#adminSpecifyProdItemDevice) that you want to see totaled on your KDS device.

Follow these procedures to complete the configuration of a production item.

**To create a production item**
...
**To assign production item quantities to a menu item or modifier**
...
**To specify the production items to report on a KDS device**
