...
excerpt: >-
  For restaurants using KDS devices, you can mark a ticket or item as fulfilled.
  When an item assigned to a single prep station is fulfilled at the prep
  station level, a green…
hidden: false
metadata:
  description: >-
    ...
    the prep station level, a green…
---
...
- [Showing other prep stations working on the items for a prep station ticket](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configOtherStations)
- [Changing how tickets group and display items](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configIndividualTickets)
- [Showing modifiers with or without their parent items](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configModifierRouting)
- [Using single or two-level expediter fulfillment](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configTwoLevelFulfillment)
- [Requiring sequential expediter fulfillment](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configSequencedExpediterFulfillment)
- [Allowing for all items on a ticket to be fulfilled at once](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configFulfillItems)

### Sequencing fulfillment by station type and level

Using KDS devices, you can configure the order in which tickets or items are fulfilled by KDS device type. You can choose to:

- Require fulfillment at all prep station KDS devices before fulfillment at the expediter KDS device using the **Sequenced Prep to Expo Fulfillment** setting.
- If using two-level fulfillment, require fulfillment at the first-level expediter KDS device before the second-level expediter KDS device using the **Sequenced Expediter Fulfillment** setting.

Fulfilling tickets or items when not fulfilled at the required level requires an override by an employee with the [Manager](https://doc.toasttab.com/doc/platformguide/adminPermissions.html#permissionManager) access permission. For information about how to sequence the order in which items are fulfilled at prep stations, see [Creating an assembly line](https://doc.toasttab.com/doc/platformguide/platformKDSCreatingAssemblyLines.html) and [Routing using assembly lines](https://doc.toasttab.com/doc/platformguide/platformKDSRoutingUsingAssemblyLines.html).
...
**To update your fulfillment method**
...
4. Select **Save**.
5. When you are ready to see these changes on the KDS devices, select **Publish Now**. If you are publishing to multiple restaurant locations, use the **Publish Config** page. For more information, see [Using the Publish Config page](https://doc.toasttab.com/doc/platformguide/platformHowToPublish.html#platformPublishingPublishConfigPage).
