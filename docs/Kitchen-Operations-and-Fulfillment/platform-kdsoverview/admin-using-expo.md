...
excerpt: >-
  In some restaurants, the kitchen staff includes an expediter. The
  responsibilities of the expediter, or expo, can include managing communication
  between the front and back of…
hidden: false
metadata:
  description: >-
    In some restaurants, the kitchen staff includes an expediter. The
    responsibilities of the expediter, or expo, can include managing
    communication between the front and back of…
---

In some restaurants, the kitchen staff includes an [*expediter*](doc:admin-glossary#glossExpediter). The responsibilities of the expediter, or expo, can include managing communication between the front and back of the house, organizing item timing, and finalizing orders for service. In this workflow, the expediter determines when an order is completely fulfilled and ready for service, rather than the cooks.
...
If your kitchen includes either one or two expediter screens, additional features are available, including options to [send SMS text messages](https://doc.toasttab.com/doc/platformguide/platformKitchenTextMessaging.html#adminSendTextMessage) to guests or servers or [send on-screen notifications](https://doc.toasttab.com/doc/platformguide/adminUsingExpo.html#adminSendOnScreenNotifications) to server devices when an expediter marks orders as fulfilled.

For more information about setting up prep stations, see [Routing with prep stations](https://doc.toasttab.com/doc/platformguide/adminRoutingToPrepStations.html).
...
To set up the Toast POS devices as expediter screens, you must also have at least one of the "mode" access permissions. For more information, see [Accessing the Toast POS Device Setup screen](https://doc.toasttab.com/doc/platformguide/adminConfigureDevice.html).

Follow one of these procedures to configure the expediter workflow.

**To configure a single expediter screen**

1. Determine which prep stations should route orders to the expediter, the prep station, or both.

   You can review the menu items and modifiers that are assigned to each prep station in Toast Web. For more information, see [Verify order routing to prep stations](https://doc.toasttab.com/doc/platformguide/adminTroubleshootPrepStationRouting.html).
2. [Access Toast Web](https://doc.toasttab.com/doc/platformguide/adminAccessToastAdminBackend.html) and choose **Kitchen > Kitchen stations > Prep stations** to open the **Prep stations** page. An interactive grid with rows for each prep station appears.
3. To send orders to the expediter and the prep station, select **Prep station and expediter** for the **Send to** setting. Configure this setting for each prep station you identified in Step 1, and then save your changes.

   > 📘 Note
   >
   > For information about having items appear only on expediter KDS devices, see [Sending items to prep stations, expediters, or both](https://doc.toasttab.com/doc/platformguide/adminRoutingToPrepStations.html#platformKitchenSendingToPrepStationExpediterBoth).
...
**To configure two expediter screens**

1. Complete Steps 1-3 from the [single expediter configuration](https://doc.toasttab.com/doc/platformguide/adminUsingExpo.html#singleExpo) procedure to define which prep stations send orders to the expediter.
2. Choose **Kitchen > Printers, tickets, & KDS devices > Kitchen and ticket setup**, and in the **Expediter** section set **Two-Level Fulfillment** to **Two levels**.
   ...
   > 📘 Note
   >
   > Confirm that the **Send to** setting is set to **Prep station and expediter** for each prep station. For information about having items appear only on expediter KDS devices, see [Sending items to prep stations, expediters, or both](https://doc.toasttab.com/doc/platformguide/adminRoutingToPrepStations.html#platformKitchenSendingToPrepStationExpediterBoth).
...
6. On the Toast POS device that is located at the *second* expediter seat, [access **Device Setup**](https://doc.toasttab.com/doc/platformguide/adminConfigureDevice.html), tap **Primary Mode**, and then make the following selections.
...
- If your kitchen workflow has [two expediters](https://doc.toasttab.com/doc/platformguide/adminUsingExpo.html), the notification is sent as soon as either of the two expediters marks the order fulfilled.
