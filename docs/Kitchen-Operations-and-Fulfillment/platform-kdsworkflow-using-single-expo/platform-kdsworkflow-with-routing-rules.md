...
excerpt: >-
  The restaurant in this example uses routing rules to change the prep stations
  for an item depending on the order's dining option and underlying dining
...
metadata:
  description: >-
    ...
    dining behavior. The example…
---

The restaurant in this example uses routing rules to change the prep stations for an item depending on the order's dining option and underlying dining behavior. The example restaurant configures all orders using the Take Out dining option to send items originally routed to the Food prep station, to instead go to the Pack prep station and Food prep station. For information on routing rules, see [Rerouting based on dining option](https://doc.toasttab.com/doc/platformguide/adminAboutItemRoutingRules.html#adminReroutingOrdersBasedOnDiningOption). This restaurant does not use courses.

This example restaurant also uses individual item fulfillment, sends prep station tickets to the expediter, uses the fulfillment method that fulfills items at the same time across multiple prep stations, and uses an outline color to differentiate KDS tickets with a dining behavior of Takeout, Delivery, or Curbside. For more information on the individual item fulfillment setting, see [Fulfill Items](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configFulfillItems). For more information about the configuration setting that sends prep station tickets to the expediter, see [Send to](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configSendTo). For information about fulfillment methods, see [Understanding item fulfillment](https://doc.toasttab.com/doc/platformguide/platformKitchenUnderstandingItemFulfillmentKDS.html). For more information on applying an outline color to tickets based on dining behavior, see [Ticket colors by dining behavior - Enabled](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configKDSTicketColorByDiningBehaviorEnabled).
...
7. At the same time, at the Pack prep station, an employee receives the Curry Bread item and packages it for transport.
   ...
   > 📘 Note
   >
   > For menu items assigned to multiple prep stations, when the item is fulfilled at one prep station, the item is fulfilled at all assigned prep stations. For more information, see [Understanding item fulfillment](https://doc.toasttab.com/doc/platformguide/platformKitchenUnderstandingItemFulfillmentKDS.html).
