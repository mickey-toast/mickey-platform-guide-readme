...
excerpt: >-
  The restaurant in this example only uses one expediter KDS device. The
  restaurant assigns items in an order to individual seats and keeps printed
  prep station tickets with the…
hidden: false
metadata:
  description: >-
    The restaurant in this example only uses one expediter KDS device. The
...
This example restaurant is configured to have a separate ticket for every menu item in an order, both for printers and on KDS devices. This allows a food runner to bring orders to the table as they are fulfilled. Printed individual tickets do not include an overview ticket at each prep station. For more information about the configuration setting that determines how items appear on printed and screen tickets, see [Individual Item Tickets](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configIndividualTickets).

This example restaurant also uses individual item fulfillment, sends prep station tickets to the expediter, and always prints tickets sent to the Food and Drink prep stations. For more information on the individual item fulfillment setting, see [Fulfill Items](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configFulfillItems). For more information about the setting that sends prep station tickets to prep station and expediter, see [Send to](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configSendTo). For more information about the setting that always prints prep station tickets, see [???](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configPrepStationsAlwaysPrintTickets).
