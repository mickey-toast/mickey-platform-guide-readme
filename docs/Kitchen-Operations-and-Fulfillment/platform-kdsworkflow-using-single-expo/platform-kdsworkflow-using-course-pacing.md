...
excerpt: >-
  The restaurant in this example uses courses and meal pacing to fire items in
  the kitchen. This example restaurant has the current configurations:
hidden: false
metadata:
  description: >-
    The restaurant in this example uses courses and meal pacing to fire items in
...
For more information on course pacing settings, see [Course Pacing](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configCoursePacingRef).
...
3. Three tickets appear on the expediter KDS device, one for each course. Only the first course, Drinks, is fired. The other course tickets are marked with **HELD**, which indicates that they are sent to the kitchen, but not fired. Tickets marked with **HELD** only appear on the expediter KDS device. The Drinks course ticket appears on Drinks prep station KDS device.

   Here are the course tickets on the expediter KDS device, with only the Drinks course ticket fired and the Entrees and Appetizers course tickets marked with **HELD**.

   ![The expediter KDS device showing three course tickets with four items, with the Drinks course ticket fired and the other course tickets on HELD.](https://doc.toasttab.com/doc/media/kitchen-kds-course-pacing-workflow-expo-new-tickets-first-course-fired.png)
