...
excerpt: >-
  For an efficient order fulfillment workflow, assign every menu item and
  modifier in your menus to a prep station. In the Toast platform, a prep
  station represents the location…
hidden: false
metadata:
  description: >-
    For an efficient order fulfillment workflow, assign every menu item and
...
After you [add your prep stations](https://doc.toasttab.com/doc/platformguide/adminAddPrepStation.html), assign them to your menus, menu groups, items, and modifiers, so that every item in an order can be sent immediately to the correct prep station for fulfillment. For items that require preparation at several prep stations, you can assign multiple prep stations.
...
> 📘 Note
>
> The limited release **Send to** setting allows you to choose whether a prep station sends its items to the prep station, expediter, or both. For more information, see [Sending items to prep stations, expediters, or both](https://doc.toasttab.com/doc/platformguide/adminRoutingToPrepStations.html#platformKitchenSendingToPrepStationExpediterBoth).
...
**To assign prep stations to send to specific station types**
...
> 📘 Note
>
> Items only need to be fulfilled at the station types they were sent to. For example, you use the **Sequenced Prep to Expo Fulfillment** setting to require fulfillment at prep station KDS devices before expediter KDS devices. This setting only applies to prep stations with the **Send to** setting set to **Prep station and expediter**. A prep station with the **Send to** setting set to **Expediter only** never sends items to a prep station KDS device, and therefore does not need to be fulfilled at a prep station KDS device first. Similarly, a prep station with **Send to** set to **Prep station only** does not require fulfillment at expediter KDS devices.

### Multiple prep stations for an item

For individual items that require work at multiple prep stations, you can choose to assign more than one prep station to it. For example, a Caesar Chicken Salad item is sent to both the *Cold* prep station and the *Grill* prep station. For more information on how the item is fulfilled at multiple prep stations on the KDS, see [Understanding item fulfillment](https://doc.toasttab.com/doc/platformguide/platformKitchenUnderstandingItemFulfillmentKDS.html).

You can also configure your kitchen tickets to show which other prep stations are working on the item or order. For more information, see [Monitoring items at other prep stations](https://doc.toasttab.com/doc/platformguide/adminMonitoringTicketsOtherPrepStations.html).

### Assigning prep stations to menu entities

The following procedure is for assigning prep stations to multiple [menu entities](https://doc.toasttab.com/doc/platformguide/adminMenuHierarchy.html) at once. You can also choose to assign prep stations to each menu entity while creating it. For more information about assigning prep stations while creating a single menu entity, see [Assigning prep stations](https://doc.toasttab.com/doc/platformguide/adminAssigningPrepStationsMenuBuilder.html).

> 📘 Note
>
> You must have the **4. Restaurant Admin > 4.5 Edit Full Menu** permission to assign prep stations to menu entities.

**To assign prep stations to menu entities**
...
5. In the **Prep Stations** column, you can assign a prep station to an entire menu, a menu group, a menu item, or a modifier option. To assign a prep station, select the **Prep Stations** cell for that menu and then select the prep station from the drop down list.
