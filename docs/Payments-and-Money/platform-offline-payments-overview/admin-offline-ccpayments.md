...
excerpt: >-
  During offline mode, you can still take card payments. When you process a card
  payment in offline mode, the Toast POS app takes the card information,
  encrypts it, and stores it…
hidden: false
metadata:
  description: >-
    During offline mode, you can still take card payments. When you process a
    card payment in offline mode, the Toast POS app takes the card information,
    encrypts it, and stores it…
---
...
**Configure card authorizations during a disruption**

1. [Access Toast Web](https://doc.toasttab.com/doc/platformguide/adminAccessToastAdminBackend.html).
2. Select **Finance > Related > Settings > Payment options**. Scroll down to the **Card authorization during payment disruptions** section.
...
4. With the **Transaction Limit** setting, you can optionally set a dollar amount limit for credit card transactions taken while in offline mode. Note that this threshold is not applied to employees who have the [Offline/Background Credit Card Processing](https://doc.toasttab.com/doc/platformguide/adminPermissions.html#permissionOfflineCCProcessing) permission. A payment larger than the configured threshold requires manager approval.
