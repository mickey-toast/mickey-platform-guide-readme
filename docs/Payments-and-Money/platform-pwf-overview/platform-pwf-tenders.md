...
excerpt: >-
  The following sections describe how to conduct cash, card, and other payments
  on your Toast POS application.
hidden: false
metadata:
  description: >-
...
**To pay a guest check with the **Cash** payment option**
...
3. The guest-facing display asks the guest to select one of the following receipt options if digital receipts are enabled:
   ...
   If **Digital Receipts** are not enabled, and a printer is configured, a **Merchant Copy** and a **Customer Copy** of the receipt are printed. For more information about **Digital Receipts** see [Digital receipts and printed receipts](https://doc.toasttab.com/doc/platformguide/platformPwfDigitalReceipts.html).
...
**To pay a guest check with the **Credit** payment option**
...
**To pay a guest check with the **Guest Pay** payment option**
...
Pre-authorizations allow restaurant employees to save a guest’s card on a check and create a tab. You must configure the Toast platform settings for your restaurant to allow credit card preauthorization. See [this Toast Support article](https://support.toasttab.com/article/Card-Pre-Authorization-Configuration-Device-Setup) for more information on enabling pre-authorizations.
...
|---|---|
| No menu items on check | The guest's account is pre-authorized for the Minimum Pre-Authorization Amount found in Toast Web at Finance > Related > Settings > Payment options. |
...
|  |
|---|
| ![](https://doc.toasttab.com/doc/media/pwfSavedCC.png) |

**To pay a guest check with the **Saved CC** payment option**
...
House accounts allow restaurant guests to make purchases and receive an invoice for payment later. You can create, manage, and edit house accounts in Toast Web from **Finance** > **Payments** > **House accounts**. For more information on creating, managing, and using house accounts, see [House accounts](https://support.toasttab.com/article/House-Accounts-1492809352564).

**To pay a guest check with the **House Acct** payment option**
...
3. Enter the guest's account number and select **Done** or select **Lookup** to [locate the guest account](https://doc.toasttab.com/doc/platformguide/platformPwfTenders.html#platformLookUpHouseAccount).
...
**How to look up a house account**
...
**To pay a guest check with the **Pay at a distance** payment option**
...
The **Other** menu includes less commonly used, or custom, payment methods. You can configure other payment methods from **Finance > Related > Settings > Other payment options** in Toast Web.

Occasionally, you may create a custom payment method that is only intended for use with Toast's APIs that you want to hide from the Toast POS app. You can control other payment methods' visibility by selecting **Yes** or **No** for the **Show on POS** setting in Toast Web at **Finance > Related > Settings > Other payment options**. Select your payment method and scroll to find **Show on POS**.

![](https://doc.toasttab.com/doc/media/pwfOtherPaymentsShowOnPos.png)
...
**To charge a hotel guest using a room charge payment**
...
> 📘 Note
>
> **Customer Credits** must be enabled in Toast Web from **Finance > Related > Settings > Customer credits**.

**To pay a guest check with the **Customer credits** payment option**
...
You can create custom payment options to fit your business. On Toast Web, navigate to **Finance > Related > Settings > Other payment options**. For more information about creating other payment types see [Setting up other payment options](https://support.toasttab.com/article/Setting-Up-Other-Payment-Options). The following sections details custom payment options that are often created by restaurant operators.

##### Gift certificate (paper)

**To pay a guest check with a custom gift certificate (paper) payment option**
...
**To pay a guest check with a custom check payment option**
...
**To pay a guest check with a custom takeout or delivery service payment option**
