...
excerpt: >-
  House accounts can hold either a balance or a credit. If a guest pays for a
  check using a house account, this creates an outstanding balance. The balance
  is the total charge…
hidden: false
metadata:
  description: >-
    House accounts can hold either a balance or a credit. If a guest pays for a
    check using a house account, this creates an outstanding balance. The
    balance is the total charge…
---
...
7. On the order screen, your payment is listed on the order details dialog. To remove a payment, select the payment on the order details pane and select the **Remove** button. This removes the payment from the check.
...
2. In the invoice, go to the **Pay this invoice** section. Select the payment method and enter the payment details.
   ...
   > 📘 Note
   >
   > If you choose to prompt guests to authorize the location to charge their payment card for future payments, guests can choose to enable auto-pay. This saves a payment card to their house account profile for future payments.

   ![Shows the Enable automatic payments section in a house account invoice.](https://doc.toasttab.com/doc/media/platform-house-accounts-automatic-payments.png)
...
6. On the **Invoice** page, choose how to pay the invoice.
   ...
   - From the **Payment schedule** link, choose one of the following methods to pay the invoice:
      ...
      ![Shows the Payment schedule link with the View payment page and the Enter a payment buttons emphasized in Toast Web.](https://doc.toasttab.com/doc/media/platform-house-accounts-payment-schedule.png)
   ...
   On the **Payment schedule** link, the payment is listed with the due date, amount paid, and the status. To view more payment details, select the down arrow next to the **Status** column. You can view payment details such as the payment method type, tip amount, payment date, and payment status.
