...
excerpt: >-
  When a guest uses a credit card to pay for an order, the credit card is
  initially charged for the order itself. The check status changes from open to
  paid. The final amount…
hidden: false
metadata:
  description: >-
    When a guest uses a credit card to pay for an order, the credit card is
    initially charged for the order itself. The check status changes from open
    to paid. The final amount…
---
...
: [Access "mode" permissions](https://doc.toasttab.com/doc/platformguide/adminPermissions.html#adminModePermissions) provide access to the areas of the Toast POS app where the close check options are located.
...
: The [Bulk Close Paid Checks](https://doc.toasttab.com/doc/platformguide/adminPermissions.html#permissionBulkClosePaidChecks) permission allows you to close multiple checks in a single operation.
...
**To close one or more paid checks**
