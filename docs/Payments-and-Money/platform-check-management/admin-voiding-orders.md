...
excerpt: >-
  Until the check payment is captured, you can void the check to cancel the
  payment and the menu item selections. For example, a guest might place an
  order and then have to leave…
hidden: false
metadata:
  description: >-
    Until the check payment is captured, you can void the check to cancel the
...
After the payment is captured, you cannot void the check. You must refund the check amount. See [Refunds and voids](https://doc.toasttab.com/doc/platformguide/adminRefundsAndVoids.html).
...
You can also [void items and orders from the Orders Hub](https://doc.toasttab.com/doc/platformguide/platformUsingOrdersHub.html#platformVoidingAnItemOrAnOrder).
...
**To configure the list of available void reasons**

1. From Toast Web, choose **Finance > Related > Settings > Void reasons**.
...
5. To archive a void reason that you no longer want to use, select the actions menu icon (**...**), then select **Archive**.
...
: [Access "mode" permissions](https://doc.toasttab.com/doc/platformguide/adminPermissions.html#adminModePermissions) provide access to the areas of the Toast POS app where the void check options are located.
...
: The [Bulk Void Open Checks](https://doc.toasttab.com/doc/platformguide/adminPermissions.html#permissionBulkVoidOpenChecks) permission allows you to select and void multiple checks at the same time.
...
******Void Items/Orders******
: The [Void Items/Orders](https://doc.toasttab.com/doc/platformguide/adminPermissions.html#permissionVoidItemsOrders) permission allows you to select and void a single check.
  If you do not have this permission, you can ask another restaurant employee who does have this permission to enter their POS access code or swipe their access card.

******Edit Other Employees' Orders******
: The [Edit Other Employees' Orders](https://doc.toasttab.com/doc/platformguide/adminPermissions.html#permissionEditOtherEmployeesOrders) permission allows you to update orders that are assigned to a different employee.
...
When a check is voided, "(VOIDED)" appears on corresponding KDS tickets with lines through the voided items. Depending on the [Print Ticket Changes](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configPrintTicketChanges) configuration, a new ticket is printed.
...
For an open order, "(VOIDED)" appears on corresponding KDS tickets with lines through the voided items. Depending on the [Print Ticket Changes](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configPrintTicketChanges) configuration, the order prints on a new ticket.
