...
excerpt: >-
  The Receipt setup page allows you to set the configuration of printed customer
  receipts. These receipts include:
hidden: false
metadata:
  description: >-
    The Receipt setup page allows you to set the configuration of printed
    customer receipts. These receipts include:
---
...
To configure receipts, you need **Restaurant Admin > POS Setup** permission to access the **Receipt setup** Toast Web page. For more information, see the [Access Permission Reference](https://doc.toasttab.com/doc/platformguide/adminPermissions.html#adminRestaurantAdminAccessPermissions).

**To configure customer printed receipts**

1. [Access Toast Web](https://doc.toasttab.com/doc/platformguide/adminAccessToastAdminBackend.html).
2. Select **Finance > Related > Settings > Guest receipt setup** to open the **Receipt setup** page.
...
4. Enter the **Basic** configuration of the receipt:
   ...
   - **Logo**: To add an image at the top of the receipt, select **Choose Image**, and go to the image file (which should be a JPEG or PNG format), and open it. The logo should be a 360-pixel square image. Note that an image that is too large will be re-sized to fit.
5. Enter the **Advanced** configuration:
   ...
   - **Print Payments**: Select whether the receipt should show all the completed payments on the customer's full check, from the **Order** screen.
      ...
      Note that if you select **Don't Show**, other payment types handled as discounts will still print on the receipt. To configure these discounted other payment types, choose **Finance > Related > Settings > Other payment options**, select the payment option name, and select **Yes** for the **Treat as Discount** option.
