---
title: Cash drawers
excerpt: 'A cash drawer can be in one of the following cash drawer states:'
hidden: false
metadata:
  description: 'A cash drawer can be in one of the following cash drawer states:'
---
...
- A lock icon next to the name of the open cash drawer if the drawer is locked to the employee. For more information, see [Cash drawer lockdown](https://doc.toasttab.com/doc/platformguide/adminCashDrawerLockdown.html).
- If the employee has the **Manager > 3.18 Cash Drawers (Full)** permission, the expected balance for all drawers is displayed in the **Expected total balance** section in the cash drawer activity pane. If the employee has the **Manager > 3.17 Cash Drawers (Blind)** permission, the expected balance for each drawer is not displayed.
...
- Cash drawer activity pane. This pane displays all cash entries and actions taken in the cash drawer.
- Cash drawer name.
- **Lock drawer to me** button. If a cash drawer is locked to an employee, a *LOCKED TO ME* banner appears at the bottom of the cash drawer activity pane and a lock icon appears next to the drawer name on the **Cash Drawers** screen.
...
- **Cash drawer actions**. For more information, see [Open cash drawer actions](https://doc.toasttab.com/doc/platformguide/adminCashDrawers.html#adminOpenCashDrawerActions).
...
For more information, see [POS cash drawer operations](https://doc.toasttab.com/doc/platformguide/adminCashDrawerPOSOperations.html).
...
- **Cash collected from server**: Adds cash collected from an employee and adds it to a cash drawer.
   ...
   > 📘 Note
   >
   > A **Cash Collected**entry is created if you undo a previous **Tip Out** entry. This includes cash collected from a server. For more information on how to undo a cash entry, see [Undoing cash drawer actions](https://doc.toasttab.com/doc/platformguide/adminUndoingOperations.html).
...
On the **Cash drawer activity** screen, you can view the cash entries for a paused cash drawer and complete cash drawer actions. The **Cash drawer activity**screen displays the same information and actions as the [Open cash drawer screen](https://doc.toasttab.com/doc/platformguide/adminCashDrawers.html#adminOpenCashDrawersScreen).

#### Paused cash drawer actions

The same cash drawer actions available for an open cash drawer are also available for a paused cash drawer. For more information, see [Open cash drawer actions](https://doc.toasttab.com/doc/platformguide/adminCashDrawers.html#adminOpenCashDrawerActions).
...
From a closed drawer, you can create a deposit. For more information, see [Cash deposits](https://doc.toasttab.com/doc/platformguide/adminCashDeposits.html).
