...
excerpt: >-
  On the Settings copy page, there is an Activity table that lists historical
  data for any copies you made for that management group. This table includes
...
- **Status**: This reflects the status of the copying. There are three possible statuses for your copy instance:
   ...
   - **COMPLETED**: The copy completed successfully.
      ...
      > 📘 Note
      >
      > In instances where a setting must account for a dependent MLM setting, if the setting is copied to a location that is not in the [location hierarchy](https://doc.toasttab.com/doc/platformguide/ownersAndPermissions.html) used by the dependent setting, the setting is not copied, but will have a **COMPLETED** status.
