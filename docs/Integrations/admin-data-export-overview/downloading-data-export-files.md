...
excerpt: >-
  The Toast platform stores data export files for seven days, and then they are
  deleted. When data exports are enabled for your restaurant, the following
  items are needed to…
hidden: false
metadata:
  description: >-
    The Toast platform stores data export files for seven days, and then they
    are deleted. When data exports are enabled for your restaurant, the
    following items are needed to…
---
...
- SSH key: For more information about generating an SSH key, and where to enter this in Toast Web, see [SSH keys](https://doc.toasttab.com/doc/platformguide/adminSshKeys.html).
...
Export IDs can also be obtained from Toast Web. Navigate to **Reports > Settings > Data Exports** and select the link in the banner at the top of the screen, **view restaurant # mapping** to export a file that includes an **Export ID** column which includes the ID for each restaurant.

![](https://doc.toasttab.com/doc/media/export_ids_data_export.png)

Prior to downloading data export files, ensure that you have [added an SSH key](https://doc.toasttab.com/doc/platformguide/adminSshKeys.html) to Toast Web. The following examples describe how to connect to your SFTP directory for use with the macOS™ terminal and Windows™ command prompt. For information about how to export files using a third party FTP solution see this [Toast Support article.](https://support.toasttab.com/article/Automated-Nightly-Data-Export-1492723819691)
