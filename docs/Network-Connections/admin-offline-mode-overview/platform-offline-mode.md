...
title: Offline mode
excerpt: >-
  Toast devices use offline mode when experiencing a connection issue. Offline
  mode indicates Toast devices communicate with each other through the Toast
...
- [Takeout and delivery orders](https://doc.toasttab.com/doc/platformguide/adminOfflineDelivery.html)
- [Toast Online Ordering](https://doc.toasttab.com/doc/platformguide/adminOfflineOnlineOrdering.html)
- [Employee clock-in and clock-out](https://doc.toasttab.com/doc/platformguide/adminOfflineEmployees.html)
- [Offline card payments](https://doc.toasttab.com/doc/platformguide/adminOfflineCCPayments.html)
...
To recover, first check that your local network cable is securely connected or Wi-Fi is enabled on the device. For more information, see [Local network disruption](https://doc.toasttab.com/doc/platformguide/adminOfflineModeOverview.html#platformOfflineNetworkDisruptionType).
...
To resolve this issue, contact your internet service provider (ISP). For more information, see [Internet service provider disruption](https://doc.toasttab.com/doc/platformguide/adminOfflineModeOverview.html#platformOfflineInternetDisruptionType).
...
- Set up a kitchen printer to print tickets that would normally display on a KDS device. For instructions, see [Adding a kitchen printer](https://doc.toasttab.com/doc/platformguide/adminAddKitchenPrinter.html).
- Contact Toast support to [temporarily disable the auto-capture process](https://doc.toasttab.com/doc/platformguide/adminOfflineCCPayments.html#adminOfflineAutoCapture) if your system does not re-establish connectivity by the end of the business day. This gives you time to make adjustments to credit card payments before they are captured. Once the payments are captured, you cannot make updates.

**Operations available in offline mode**

- Print kitchen tickets to backup printers (if configured). For more information, see the [Printing Mode](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configPrintingMode) and [Always Print Tickets](https://doc.toasttab.com/doc/platformguide/adminKitchenDiningRoomReference.html#configPrepStationsAlwaysPrintTickets) settings.

   > 📘 Note
   >
   > Kitchen workflows use *offline mode with local sync*, unless an error is detected by the KDS. For more information, see [KDS devices](https://doc.toasttab.com/doc/platformguide/platformOfflineKDSDevices.html).
- Use scales for weighted menu items.
- Process credit cards as offline payments. For more information, see [Offline card payments](https://doc.toasttab.com/doc/platformguide/adminOfflineCCPayments.html).
...
- Display POS and Kiosk orders on KDS devices.
   ...
   > 📘 Note
   >
   > Kitchen workflows automatically use *offline mode with local sync*, unless an error is detected by the KDS. For more information, see [KDS devices](https://doc.toasttab.com/doc/platformguide/platformOfflineKDSDevices.html).
...
- Look up guest information for takeout or delivery orders. For details, see [Takeout and delivery orders](https://doc.toasttab.com/doc/platformguide/adminOfflineDelivery.html).
